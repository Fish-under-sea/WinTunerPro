...
# 配置导入：Nexus 设置存于注册表 HKCU\Software\Winstep。预置配置以 .reg 形式提供，
# 导入前先备份现有 HKCU\Software\Winstep，再 reg import。

param(
  [string]$InstallerPath = '',   # 离线安装 exe 路径
  [string]$ConfigSource = '',    # 预置配置（.reg），导入 HKCU\Software\Winstep
...
try {
  ...
  # 配置导入（可选）：.reg 文件，导入前备份现有 Winstep 注册表
  ...
  if (-not [string]::IsNullOrWhiteSpace($ConfigSource)) {
    ...
    if ($ext -eq '.reg') {
      $backup = Backup-RegistryKeyToFile -RegPath 'HKCU\Software\Winstep' -BackupDir $BackupDir -Tag 'nexus-winstep'
